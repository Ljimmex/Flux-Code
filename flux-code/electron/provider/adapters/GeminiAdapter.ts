...
export class GeminiAdapter implements ProviderAdapter {
  ...
  async probe(): Promise<ProviderStatus> {
    ...
    try {
      ...
      return { kind: 'not-authenticated', installCmd: 'gemini auth login', models: MODELS };
    }
  }
  ...
  async startSession(opts: SessionStartOpts): Promise<void> {
    ...
    proc.on('exit', (code) => {
      if (code !== 0 && code !== null) {
        this.emit({ type: 'session.error', sessionId: opts.sessionId, message: `Gemini exited with code ${code}` });
      }
